...
//tfnew bool: LoomynartyMusic = false;
	...
	if(!noMelee)
	{
		if (!(BMO_Flags & BMO_FLAG_KEEP_MELEE))
		{
			...
			switch(DifficultyLevel)
			{
				...
				default: Format(blitzAtts, sizeof(blitzAtts), "%i ; %f ; 137 ; 8.77 ; 214 ; %i ; 267 ; 1 ; 2025 ; 3 ; 2013 ; 2007 ; 2014 ; 6", GetDamageValue(DifficultyLevel, DiffMultiplier)<1 ? 1 : 2, GetDamageValue(DifficultyLevel, DiffMultiplier), bmKills[client]);
			}
...
public Hook_OnRocketSpawn(entity)
{
	...
	{
		SetEntPropFloat(entity, Prop_Send, "m_flPlaybackRate", 11.0);
		rocketCount++;
	}
}
...
public int RandomDanmaku(int client, int difficulty, bool isBarrage)
{
	...
	*/
	RNGDamage = GetRandomFloat(1.0, 1.3), RNGSpeed = GetRandomFloat(0.6, 0.8);
	/*
	if(!isBarrage)
	{
	...
	*/
	...
	TF2Items_SetAttribute(hItem, 0, 97, 0.01); // Reload Speed
	// TF2Items_SetAttribute(hItem, 1, isBarrage && RNGesus ? 521 : 542, 1.0); // Mini-Projectiles
	TF2Items_SetAttribute(hItem, 1, 413, 1.0); // Press & Hold to reload
	TF2Items_SetAttribute(hItem, 2, 2025, 3.0); //Is Pro Killstreak
	TF2Items_SetAttribute(hItem, 3, 2013, GetRandomFloat(2002.0, 2008.0)); // Killstreaker
	TF2Items_SetAttribute(hItem, 4, 2014, GetRandomFloat(1.0, 7.0)); // Sheen
	TF2Items_SetAttribute(hItem, 5, 4, GetRandomFloat(10.0, 60.0)); // Clip Size
	TF2Items_SetAttribute(hItem, 6, 6, GetRandomFloat(0.1, 0.3)); // Fire rate bonus
	TF2Items_SetAttribute(hItem, 7, 37, 0.0); // Cannot pick up ammo
	TF2Items_SetAttribute(hItem, 8, (RNGDamage >= 1.0 ? 2 : 1), RNGDamage); // Damage Bonus/Penalty
	TF2Items_SetAttribute(hItem, 9, (RNGSpeed >= 1.0 ? 103 : 104), RNGSpeed); // Speed Bonus/Penalty
	TF2Items_SetAttribute(hItem, 10, (RNGSpread > 0 ? 411 : 269), (RNGSpread > 0 ? RNGSpread : 1.0)); // Spread/See Enemy Health
