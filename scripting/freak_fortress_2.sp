...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	OnMusic=CreateGlobalForward("FF2_OnMusic", ET_Hook, Param_String, Param_FloatByRef, Param_String, Param_String, Param_Cell);
...
public OnPluginStart()
{
	...
	if(!HookEventEx("player_death", OnPlayerDeath, EventHookMode_Pre)) //  첫라운드 버그가 문제야..
	{
		LogMessage("이 이벤트 이상해요!");
	}
...
*/
	...
	for(new client; client<=MaxClients; client++)
	{
		if(IsBoss(client))
		{
			...
			new bool:hasSpecialApproach=specialApproach[0]!='\0';
			if(BossLives[boss]>1)
			{
			...
			Format(text, sizeof(text), "%s\n%t", text, "ff2_start", Boss[boss], name, BossHealth[boss]-BossHealthMax[boss]*(BossLives[boss]-1), lives, hasSpecialApproach ? specialApproach : "");
			...
			new String:diff[25];
			new bool:gotHard=false;
			switch(BossDiff[boss]) // TODO: 이걸 함수로 한번에.
			{
			  case 2:
			  {
					Format(diff, sizeof(diff), "어려움");
					gotHard=true;
			  }
				case 3:
				{
					Format(diff, sizeof(diff), "매우 어려움");
					gotHard=true;
				}
				case 4:
				{
					Format(diff, sizeof(diff), "너무 어려움");
					gotHard=true;
				}
				case 5:
				{
					Format(diff, sizeof(diff), "불지옥");
					gotHard=true;
				}
			}
			if(gotHard) CPrintToChatAll("{olive}[FF2]{default} 이 보스는 난이도가 {green}%s{default}입니다!", diff);
		}
	}
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	{
		new Handle:itemOverride=PrepareItemHandle(item, _, _, "10 ; 1.75 ; 11 ; 1.5 ; 144 ; 2.0 ; 199 ; 0.75 ; 499 ; 1 ; 547 ; 0.75", true);
	...
	if(IsBoss(client) || GetGameTime()>=HPTime)
	{
		...
		for(new target; target<=MaxClients; target++)
		{
			if(IsBoss(target))
			{
				...
				Format(text, sizeof(text), "%s\n%t", text, "ff2_hp", name, BossHealth[boss]-BossHealthMax[boss]*(BossLives[boss]-1), BossHealthMax[boss], lives, diffItem);
				CPrintToChatAll("{olive}[FF2]{default} %t", "ff2_hp", name, BossHealth[boss]-BossHealthMax[boss]*(BossLives[boss]-1), BossHealthMax[boss], lives, diffItem);
...
public Action:ClientTimer(Handle:timer)
{
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			if(!IsPlayerAlive(client))
			{
				...
				if(IsValidClient(observer))
				{
					...
					else if(IsBoss(observer) && observer!=client)
					{
						...
						if(BossLives[boss]>1)
						{
							Format(temp, sizeof(temp), "%t", "ff2_client_timer_observer_lifes", BossHealth[boss]/BossLives[boss], BossHealthMax[boss], BossLives[boss]);
						}
						else
						{
			...
			if((!StrContains(classname, "tf_weapon_smg") && index!=751) ||  //Cleaner's Carbine
			...
			else
			{
				...
				if(BossAbilityDuration[boss] > 0.0 || BossAbilityCooldown[boss] > 0.0)
				{
					...
					if(BossAbilityDuration[boss] > 0.0)
					{
						...
						Format(temp, sizeof(temp), "%t", "rage_meter_duration", BossAbilityName[boss], temp2);
					}
					else if(BossAbilityCooldown[boss] > 0.0)
					{
						Format(temp2, sizeof(temp2), "%.1f", BossAbilityCooldown[boss]);
						SetHudTextParams(-1.0, 0.83, 0.15, 255, 255, 255, 255);
						Format(temp, sizeof(temp), "%t", "rage_meter_cooldown_easy", temp2);
					}
				}
				else
				{
		...
		else
		{
			...
			else if(BossAbilityDuration[boss] > 0.0 || BossAbilityCooldown[boss] > 0.0)
			{
				...
				if(BossAbilityDuration[boss] > 0.0)
				{
					...
					SetHudTextParams(-1.0, 0.83, 0.15, 64, 255, 64, 255);
					FF2_ShowSyncHudText(client, rageHUD, "%t", "rage_meter_duration", BossAbilityName[boss], temp);
				}
				else if(BossAbilityCooldown[boss] > 0.0)
				{
					Format(temp, sizeof(temp), "%.1f", BossAbilityCooldown[boss]);
					FF2_ShowSyncHudText(client, rageHUD, "%t", "rage_meter_cooldown_easy", temp);
				}
			}
		...
		if(RedAlivePlayers==1)
		{
			...
			for(new target; target<=MaxClients; target++)  //TODO: Why is this for loop needed when we're already in a boss for loop
			{
				if(IsBoss(target))
				{
					...
					new String:diffItem[50];
					GetDifficultyString(BossDiff[boss], diffItem, sizeof(diffItem));
					Format(message, sizeof(message), "%s\n%t", message, "ff2_hp", name, BossHealth[boss2]-BossHealthMax[boss2]*(BossLives[boss2]-1), BossHealthMax[boss2], bossLives, diffItem);
				}
			}
