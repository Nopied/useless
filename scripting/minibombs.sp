...
public Action:Timer_SetProps(Handle:timer, any:entity)
{
	...
	if (ent > 0 && ent > MaxClients && IsValidEntity(ent) && GetEntityClassname(ent, sClass, sizeof(sClass)))
	{
		if(StrEqual(sClass, "tf_projectile_pipe_remote") && GetEntProp(ent, Prop_Send, "m_bTouched") && GetEntProp(ent, Prop_Send, "m_iType") != 2)
		{
			...
			new melee=GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
			new index=((IsValidEntity(melee) && melee>MaxClients) ? GetEntProp(melee, Prop_Send, "m_iItemDefinitionIndex") : -1);

			if((index == 265 || FF2_GetFF2Userflags(client) & FF2USERFLAG_ALLOW_MINIBOMB)
			&& !(FF2_GetFF2Userflags(client) & FF2USERFLAG_NOTALLOW_MINIBOMB))
			{
