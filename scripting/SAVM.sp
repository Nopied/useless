...
public bool GetUserDatabaseData(Handle db, char[] steamID, int client)
{
	...
	if (hQuery != INVALID_HANDLE)
	{
		...
		while (SQL_FetchRow(hQuery))
		{
			...
			else
			{
				...
				// POTRY
				for(int count=1; count<VIPEFFECT_MAX_COUNT; count++)
				{
					POTRY_EnableClientVIPEffect(client, view_as<VIPEffect>(count), false);
				}
			}
	...
	if (hQuery != INVALID_HANDLE)
	{
		...
		while (SQL_FetchRow(hQuery))
		{
			...
			if(isDateGreater(today, registeredDay))
...
public int CanUseTrial(Handle db, char[] steamID, int client, char[] expirationdate)
{
	...
	if (hQuery != INVALID_HANDLE)
	{
		...
		while (SQL_FetchRow(hQuery))
		{
			...
			if(isDateGreater(DateExpiration, today))
			{
				CloseHandle(hQuery);
