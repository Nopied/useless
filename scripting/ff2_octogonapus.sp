	...
	if(FF2_GetRoundState() == 1)	// Late-load
	{
		...
		for(int iIndex; GetClientOfUserId(FF2_GetBossUserId(iIndex))>0; iIndex++)
		{
			if(FF2_HasAbility(iIndex, this_plugin_name, "rage_octogonapus"))
			{
				...
				g_iBeamR = FF2_GetAbilityArgument(iIndex, this_plugin_name, "rage_octogonapus", 8, 255);
				g_iBeamG = FF2_GetAbilityArgument(iIndex, this_plugin_name, "rage_octogonapus", 9, 255);
				g_iBeamB = FF2_GetAbilityArgument(iIndex, this_plugin_name, "rage_octogonapus", 10, 255);
...
public Action Event_PlayerDeath(Event hEvent, const char[] sName, bool bDontBroadcast)
{
	...
	int iBoss = FF2_GetBossUserId(iAttacker);

	if(iAttacker > MaxClients && GetEntPropEnt(iAttacker, Prop_Send, "m_hOwnerEntity") == iBoss)
	{
...
void Rage_Octogonapus(int iIndex, const char[] ability_name)
{
	...
	TF2_AddCondition(iBoss, TFCond_Ubercharged, -1.0);
	...
	Handle hTrace = TR_TraceRayFilterEx(vStart, vAng, CONTENTS_SOLID, RayType_Infinite, TraceFilterEverything, iBoss);
...
public Action OctogonapusBlast(Handle hTimer, Handle hPack)
{
	...
	while(!TR_PointOutsideWorld(vEnd))
	{
		...
		Handle hTrace = TR_TraceHullFilterEx(vMiddle, vMiddle, g_vMins, g_vMaxs, MASK_SHOT, TraceFilterEverything, iBoss);
		CopyVector(vEnd, vStart);

		int iClient = TR_GetEntityIndex(hTrace);
...
public Action OctogonapusReset(Handle hTimer, any iBoss)
{
	...
	TF2_RemoveCondition(iBoss, TFCond_Ubercharged);
}
...
public bool TraceFilterEverything(int iEntity, int fMask, any iBoss)
{
	if(iEntity == iBoss)
		return false;
