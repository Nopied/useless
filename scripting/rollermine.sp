...
{
	CreateNative("ROLLER_CreateRollerMine", Native_CreateRollerMine);
	...
	if(client > 0 && client <= MaxClients && IsClientInGame(client))
	{
		...
		if(TR_DidHit(trace))
		{
			...
			if(IsValidEntity(iEnt))
			{
				...
				RollerMineTeam[iEnt] = GetClientTeam(client);
...
stock int Entity_GetClosestClient(int iEnt)
{
	...
	{
		if(IsClientInGame(i) && Entity_Cansee(iEnt, i) && IsPlayerAlive(i) && RollerMineTeam[iEnt] != GetClientTeam(i))
		{
			float flPos2[3];
...
public Native_CreateRollerMine(Handle plugin, numParams)
{
