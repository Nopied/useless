        ...
        for(new i=1; i<=MaxClients; i++)
        {
          if(IsValidClient(i) && IsPlayerAlive(i) && i != client)
          {
            ...
            if(TF2_GetClientTeam(i) != team && GetVectorDistance(playerPos, vSapperPos) <= GetConVarFloat(g_cvSapRadius))
            {
              ...
              SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 0.4);
              SetEntPropFloat(weapon, Prop_Send, "m_flNextSecondaryAttack", GetGameTime() + 0.4);
              SetEntPropFloat(i, Prop_Send, "m_flNextAttack", GetGameTime() + 0.4);
            }
          }
        }
    }
