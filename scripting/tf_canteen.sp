...
public Action OnPlayerSpawn(Handle event, const char[] name, bool dont)
{
	...
	if(!IsBossTeam(client))
	{
		int bottle = FindBottle(client);
		...
		if (bottle != -1)
		{
			...
			static const char abilities[4][54] = {"ubercharge", "critboost", "building instant upgrade", "refill_ammo"};

			SetEntProp(bottle, Prop_Send, "m_usNumCharges", 1); // m_bActive
			TF2Attrib_SetByDefIndex(bottle, 271, 1.0);
			TF2Attrib_SetByName(bottle, abilities[GetRandomInt(0, 3)], 1.0);

			SetEntProp(bottle, Prop_Send, "m_bActive", 1); // m_bValidatedAttachedEntity
			SetEntProp(bottle, Prop_Send, "m_bValidatedAttachedEntity", 1);
			SetEntProp(bottle, Prop_Send, "m_bInitialized", 1);
			SetEntProp(bottle, Prop_Send, "m_bOnlyIterateItemViewAttributes", 1);
