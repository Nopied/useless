...
public Action:EnablePod(Handle:timer, any:ref)
{
    ...
    if(ent != INVALID_ENT_REFERENCE)
    {
        ...
        SDKHook(ent, SDKHook_Touch, OnTouchBeam);
        SDKHook(ent, SDKHook_EndTouch, OnEndTouchBeam);
    }
}
...
public Action:OnTouchBeam( brush, entity )
{
    ...
    {
        GetEntPropVector(entity, Prop_Send, "m_vecOrigin", clientpos);
        GetEntPropVector(brush, Prop_Send, "m_vecOrigin", beampos);
        ...
        TeleportEntity(entity, NULL_VECTOR, clientpos, clientpos);
    }
    return Plugin_Continue;
}
