...
public Native_GetState(Handle plugin, numParams)
{
	int client = GetNativeCell(1);
	AimbotType type = GetNativeCell(2);

	if(type == AimbotType_Aimbot)
	{
		return g_bAimbot[client];
	}
	else if(type == AimbotType_AutoShoot)
	{
		return g_bAutoShoot[client];
	}
	else if(type == AimbotType_NoSpread)
	{
		return g_bAutoShoot[client];
	}
	else if(type == AimbotType_SlientAim)
	{
...
public Native_SetState(Handle plugin, numParams)
{
	int client = GetNativeCell(1);
	AimbotType type = GetNativeCell(2);
	...
	if(type == AimbotType_Aimbot)
	{
		g_bAimbot[client] = GetNativeCell(3);
	}
	else if(type == AimbotType_AutoShoot)
	{
		g_bAutoShoot[client] = GetNativeCell(3);
	}
	else if(type == AimbotType_NoSpread)
	{
		g_bAutoShoot[client] = GetNativeCell(3);
	...
	{
		g_bSilentAim[client] = GetNativeCell(3);
	}
}
