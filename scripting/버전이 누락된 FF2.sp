...
public Action:Timer_Announce(Handle:timer)
{
	...
	if(Announce>1.0 && Enabled2)
	{
		switch(announcecount)
		{
			case 1:
			{
				CPrintToChatAll("{LightSeaGreen}[POTRY]{default} %t", "Forpotry_a");
			}
			case 3:
			{
				CPrintToChatAll("{LightSeaGreen}[POTRY]{default} %t", "Forpotry_b");
			}
			case 4:
			{
				CPrintToChatAll("{LightSeaGreen}[POTRY]{default} %t", "Forpotry_c");
			}
			case 5:
			{
				announcecount=0;
			...
			default:
			{
				CPrintToChatAll("{LightSeaGreen}[POTRY]{default} %t", "type_ff2_to_open_menu");
			}
		}
	}
	return Plugin_Continue;
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new client; client<=MaxClients; client++)
	{
		if(IsValidClient(client))
		{
			...
			if(IsBoss(client))
			{
				FF2_ShowSyncHudText(client, infoHUD, "\n%t\n\n1) %i-%s\n2) %i-%s\n3) %i-%s\n", "top_3", Damage[top[0]], leaders[0], Damage[top[1]], leaders[1], Damage[top[2]], leaders[2]);
			}
			else
			{
				FF2_ShowSyncHudText(client, infoHUD, "\n%t\n\n1) %i-%s\n2) %i-%s\n3) %i-%s",  "top_3", Damage[top[0]], leaders[0], Damage[top[1]], leaders[1], Damage[top[2]], leaders[2]);
			}
		}
	}
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client))
		{
			...
			else if(!IsFakeClient(client) && (GetClientTeam(client)>_:TFTeam_Spectator || SpecForceBoss))
			{
				...
				{
					add_points[client]=1;
					add_points2[client]=1;
				}
				else
				{
					add_points[client]=0;
					add_points2[client]=0;
				}
			}
		}
	}
		...
		case Plugin_Changed:
		{
			for(new client=1; client<=MaxClients; client++)
			{
				if(IsValidClient(client))
				{
					...
					{
						 if(add_points2[client]==3) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points3");
						
						 if(add_points2[client]==2) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points2");
						
						 if(add_points2[client]==1) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points1");
						
						 if(add_points2[client]==0) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points");
					}
			...
			for(new client=1; client<=MaxClients; client++)
			{
				if(IsValidClient(client))
				{
					...
					if(add_points[client]>0) 
					{
						 ...
						 if(add_points[client]==3) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points3");
						
						 if(add_points[client]==2) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points2");
						
						 if(add_points[client]==1) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points1");
						
						 if(add_points[client]==0) CPrintToChat(client, "{olive}[FF2]{default} %d %t", mage, "add_points");
					}
...
public Action:StartBossTimer(Handle:timer)
{
	...
	CreateTimer(0.2, StartRound); // 타이머 시작
	CreateTimer(0.2, ClientTimer, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(2.0, Timer_MusicPlay, 0, TIMER_FLAG_NO_MAPCHANGE);
	
	timeleft = 90 + (RedAlivePlayers + BlueAlivePlayers)*12;
	// 분노
...
/*
public Action:RageTimer(Handle:timer)
{
	for(new client; client<=MaxClients; client++)
	{
		new boss = GetBossIndex(client);
		
		if(IsValidClient(boss) && IsBoss(boss) && executed3)
		{
			BossCharge[boss][0] =  BossCharge[boss][0]++;
...
public Action:StartRound(Handle:timer)
{
	...
	CreateTimer(2.0, Timer_alreadyisaid);
	UpdateHealthBar();
	return Plugin_Handled;
}

public Action:Timer_alreadyisaid(Handle:timer)
{
	if(!alreadyisaid && RedAlivePlayers < 6) 
	{

	CPrintToChatAll("{olive}[FF2]{default} 레드팀의 인원이 6명 미만이면 타이머가 작동되지 않습니다.");
	alreadyisaid = true;
...
public Action:CheckItems(Handle:timer, any:client)
{
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			...
			case 402:  //Bazaar Bargain 무기: 시장 흥정품
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
	...
	new playerBack=FindPlayerBack(client, 57);  //Razorback 무기: 레이져 백 
	...
	if(tf2attributes)
	{
		if(IsValidEntity(FindPlayerBack(client, 444)))  //Mantreads 무기: 인간 딛개 
		{
	...
	new entity=-1;
	while((entity=FindEntityByClassname2(entity, "tf_wearable_demoshield"))!=-1)  //Demoshields 무기: 방패
	{
	...
	if(civilianCheck[client]==3)
	{
		...
		CPrintToChat(client, "{olive}[FF2]{default} 무기가 없어서 리스폰합니다.");
		TF2_RespawnPlayer(client);
	}
	civilianCheck[client]=0;
	return Plugin_Continue;
	...
	if(Enabled && IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		if(IsValidEntity(medigun))
		{
			...
			if(!strcmp(classname, "tf_weapon_medigun"))
			{
				...
				SetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel", 2.20);
...
public Action:Timer_ResetUberCharge(Handle:timer, any:medigunid)
{
	...
	if(IsValidEntity(medigun))
	{
		SetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel", GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel"));
	}
	return Plugin_Continue;
}
...
public Action:Command_SetNextBoss(client, args)
{
	...
	if(args<1)
	{
		CReplyToCommand(client, "{olive}[FF2]{default} 방법: ff2_special <boss>");
		return Plugin_Handled;
	}
	...
	for(new config; config<Specials; config++)
	{
		...
		{
			Incoming[0]=config;
			CReplyToCommand(client, "{olive}[FF2]{default} 다음 보스를 %s로 설정합니다.", boss);
			return Plugin_Handled;
		}
	...
	CReplyToCommand(client, "{olive}[FF2]{default} 그 보스는 있지않습니다!!");
	return Plugin_Handled;
}
...
public Action:Command_Points(client, args)
{
	...
	if(args!=2)
	{
		CReplyToCommand(client, "{olive}[FF2]{default} 방법: ff2_addpoints <target> <points>");
		return Plugin_Handled;
	}
...
public Action:Command_StopMusic(client, args)
{
	if(Enabled2)
	{
		...
		{
			StopMusic();
			CReplyToCommand(client, "{olive}[FF2]{default} 보스 BGM을 껐습니다.");
		}
		return Plugin_Handled;
	}
	return Plugin_Continue;
...
public Action:ClientTimer(Handle:timer)
{
	...
	new TFCond:cond;
	// 소생 타이머
	// if(afterroundstart = 1) LiveTimer = HookEvent("player_spawn", when_player_spawn, EventHookMode_PostNoCopy);
	
	// if(!(LiveTimer=INVALID_HANDLE)) BlockRPM = true;
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			if(!IsPlayerAlive(client))
			{
				...
				{
					FF2_ShowSyncHudText(client, rageHUD, "피해량: %d - %N님의 피해량: %d", Damage[client], observer, Damage[observer]);
				}
				if(IsBoss(observer))
				{
					new boss = GetBossIndex(observer);
					
					if(BossLives[boss] > 1) FF2_ShowSyncHudText(client, rageHUD, "HP: %d / %d x%d | 분노: %d / 100", BossHealth[boss]/BossLives[boss] ,BossHealthMax[boss], BossLives[boss] ,RoundFloat(BossCharge[boss][0]));
					
					else FF2_ShowSyncHudText(client, rageHUD, "HP: %d / %d | 분노: %d / 100", BossHealth[boss] ,BossHealthMax[boss] ,RoundFloat(BossCharge[boss][0]));
					
				}
				continue;
			}
			FF2_ShowSyncHudText(client, rageHUD, "피해량: %d", Damage[client]);
...
public Action:when_player_spawn(Handle:event, const String:Name[], bool:dontBroadcast)
{	
	if (BlockRPM=true)
	{
		CPrintToChatAll("{olive}[FF2]{default} %t", "who_alive_to_medic");
		timeleft = timeleft + 5;
	}
...
public Action:BossTimer(Handle:timer)
{
	...
	for(new client; client<=MaxClients; client++)
	{
		...
		// CreateTimer(2.0, RageTimer, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
		...
		if(RedAlivePlayers==1)
		{
			...
			for(new target; target<=MaxClients; target++)
			{
				if(IsBoss(target))
				{
					...
					if(BossLives[boss]>1)
					{
						Format(message, sizeof(message), "%s\n%s(이)의 HP: %i / %ix%i", message, name, BossHealth[boss]-BossHealthMax[boss]*(BossLives[boss]-1), BossHealthMax[boss], BossLives[boss]);
					}
					else
					{
						Format(message, sizeof(message), "%s\n%s(이)의 HP: %i / %i", message, name, BossHealth[boss]-BossHealthMax[boss]*(BossLives[boss]-1), BossHealthMax[boss]);
					}
				}
			}
...
public TF2_OnConditionAdded(client, TFCond:condition)
{
	if(Enabled && IsBoss(client) && condition==TFCond_Jarated || condition==TFCond_MarkedForDeath || (condition==TFCond_Dazed && TF2_IsPlayerInCondition(client, TFCond:42)))
	{
		...
		{
			BossCharge[boss][0]-=10.0;  //TODO: Allow this to be customizable
...
public Action:CheckAlivePlayers(Handle:timer)
{
	...
	else if(RedAlivePlayers==1 && BlueAlivePlayers && Boss[0] && !RoundTimer)
	{
		decl String:sound[PLATFORM_MAX_PATH];
	...
	// 서든타이머를 여기서 써먹음.
	else if(!PointType && RedAlivePlayers<=AliveToEnable && !executed && !SuddenTimer)
	{
	...
	if(RedAlivePlayers <= 32 && !executed2  &&  RedAlivePlayers >= 6)
	{
		if(RedAlivePlayers <= 5) CPrintToChatAll("{olive}[FF2]{default} %t", "Not_enough_people");
		if(FindEntityByClassname2(-1, "team_control_point")!=-1)
		{
			timeleft = 90 + (RedAlivePlayers + BlueAlivePlayers)*12;
			RoundTimer=CreateTimer(1.0, Timer_DrawGame, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
		}
		executed2=true;
	}
...
public Action:Timer_DrawGame(Handle:timer)
{
	...
	// 서든데스 타이머	
		case 0:
		{
			SuddenTimer = CreateTimer(1.0, Timer_Sudden, _,TIMER_REPEAT); SuddenDeath = true;
			return Plugin_Stop;
...
//서든데스 타이머
public Action:Timer_Sudden(Handle:timer)
{
	if (!SuddenDeath) return Plugin_Stop;
	
	PrintCenterTextAll("서든데스 상태이므로 양팀의 HP가 서서히 깎입니다!");
	...
	for(new client; client<=MaxClients; client++)
	{
		...
		{
			new boss = GetBossIndex(client);
			new wastedHP = BossHealthMax[boss]/100;
			new HP = BossHealth[boss] - wastedHP;
			BossHealth[boss] = HP;
		...
		if(IsValidClient(client) && IsPlayerAlive(client) && !IsBoss(client) && TF2_GetClientTeam(client) == TFTeam_Red)
		{
			new String:Name[256];
			new hp = GetClientHealth(client);
			
			SetEntProp(client, Prop_Data, "m_iHealth", hp - 10);
			SetEntProp(client, Prop_Send, "m_iHealth", hp - 10);
			if (GetClientHealth(client) <= 0) 
			{
				GetClientName(client, Name, 256);
				ForcePlayerSuicide(client);
				CPrintToChatAll("{olive}[FF2] {red}%s{default}님의 허리에 힘이 풀려 사망하셨습니다.", Name);
			...
			if(attacker<=MaxClients)
			{
				...
				if(bIsTelefrag)
				{
					...
					if(IsValidClient(teleowner) && teleowner!=attacker)
					{
						...
						{
							PrintHintText(teleowner, "크으.. 텔레프래그! 참 잘하셨어요!");
						}
					}

					if(!(FF2flags[attacker] & FF2FLAG_HUDDISABLED))
					{
						PrintHintText(attacker, "크으.. 텔레프래그! 참 잘하셨어요!");
					}

					if(!(FF2flags[client] & FF2FLAG_HUDDISABLED))
					{
						PrintHintText(client, "텔레프래그에 걸리셨어요! 다음부턴 조심하세요!");
					}
					return Plugin_Changed;
				}
				...
				switch(index)
				{
					...
					case 416:  //Market Gardener (courtesy of Chdata) 마켓가든
					{
						...
						damage=(BossHealthMax[boss]*0.08)/3;
						damagetype|=DMG_CRIT;
				...
				if(bIsBackstab)// 백스탭
				{
					...
					new String:attackerName[32];
					damage=(BossHealthMax[boss]*0.10)/3;
					damagetype|=DMG_CRIT;
					damagecustom=0;
					...
					{
						new health=GetClientHealth(attacker)+250;
						if(health>500)
						{
							health=500;
						}
...
public Action:OnStomp(attacker, victim, &Float:damageMultiplier, &Float:damageBonus, &Float:JumpPower) // 굼바
{
	...
	if(IsBoss(attacker))
	{
		...
		damageMultiplier=9000.0;
		JumpPower=0.0;
		PrintHintText(victim, "머리 조심하세요!");
		PrintHintText(attacker, "누군가를 굼바하셨어요!");
		return Plugin_Changed;
	}
	else if(IsBoss(victim))
	{
		...
		damageMultiplier=(BossHealthMax[victim]*0.06)*720;
		JumpPower=reboundPower;
		PrintHintText(victim, "머리 조심하세요! 굼바당하셨어요!");
		PrintHintText(attacker, "보스를 굼바하셨어요!");
...
stock RandomlyDisguise(client)	//Original code was mecha's, but the original code is broken and this uses a better method now.
{
	if(IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		{
			TF2_DisguisePlayer(client, TFTeam_Red, classArray[class], disguiseTarget);
		}
		else
		{
...
public Action:HelpPanelClass(client)
{
	...
	switch(class)
	{
		...
		case TFClass_Soldier:
		{
			Format(text, sizeof(text), "%t", "Hello");
		}
		case TFClass_Pyro:
		{
			Format(text, sizeof(text), "%t", "Hello");
...
HelpPanelBoss(boss)
{
	...
	Format(language, sizeof(language), "description_en", language);
...
public MusicTogglePanelH(Handle:menu, MenuAction:action, param1, param2)
{
	if(IsValidClient(param1))
	{
		if(action==MenuAction_Select)
		{
			...
			CPrintToChat(param1,"{olive}[FF2]{default} %t","ff2_music", param2==2 ? "끄기" : "켜기");
		}
	}
}
...
public Action:VoiceTogglePanel(client)
{
	...
	new Handle:panel=CreatePanel();
	SetPanelTitle(panel, "보스 효과음/대사를...");
	DrawPanelItem(panel, "켜기");
	DrawPanelItem(panel, "끄기");
...
public VoiceTogglePanelH(Handle:menu, MenuAction:action, client, selection)
{
	if(IsValidClient(client))
	{
		if(action==MenuAction_Select)
		{
			...
			CPrintToChat(client, "{olive}[FF2]{default} %t", "ff2_voice", selection==2 ? "끄기" : "켜기");
			if(selection==2)
			{
