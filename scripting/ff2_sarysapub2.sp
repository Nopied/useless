...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (WA_ActiveThisRound)
	{
		...
		for (new clientIdx = 0; clientIdx < MAX_PLAYERS; clientIdx++)
		{
			...
			{
				PrintCenterText(clientIdx, "만약 물 텍스쳐가 깨져서 아무것도 보지 못할 경우, \n!nooverlay 명령어로 제거해주시길 바랍니다.");
				CPrintToChat(clientIdx, "{black}만약 물 텍스쳐가 깨져서 아무것도 보지 못할 경우, \n!nooverlay 명령어로 제거해주시길 바랍니다.");
			}
		}
...
stock Float:RandomNegative(Float:ie)
{
	return ie * (GetRandomInt(0, 1) == 1 ? 1.0 : -1.0);
}
